...
export async function createProduct(req, res) {
  try {
    ...
    // gets the unique image urls from secure_url
...
export async function updateProduct(req, res) {
  try {
    // use 'id' based on whats the id parameter in the route: "/products:id"
    const { id } = req.params;
...
export async function updateOrderStatus(req, res) {
  try {
    ...
    // if status is shipped and there is no shipment date yet, add a new date
    if (status === "shipped" && !order.shippedAt) {
      order.shippedAt = new Date();
    }

    // if status is delivered and there is no delivered date yet, add a new date
